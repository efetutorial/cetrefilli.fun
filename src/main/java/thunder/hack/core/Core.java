...
public final class Core {
    ...
    public void onTick(PlayerUpdateEvent event) {
        ...
        if (ModuleManager.clickGui.getBind().getKey() == -1) {
            Command.sendMessage(Formatting.RED + "Varsayılan clickgui tuş ataması --> P");
            Command.sendMessage(Formatting.RED + "Hazır bir yapılandırma almak için aşağıdaki komutu çalıştırabilirsiniz --> @cfg cloudlist.");
        ...
        if (autoSave.every(600000)) {
            ...
            Managers.NOTIFICATION.publicity("AutoSave", "Yapılandırma kaydediliyor..", 3, Notification.Type.INFO);
        }
    }
