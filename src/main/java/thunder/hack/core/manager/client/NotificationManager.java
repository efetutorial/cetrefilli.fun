...
public class NotificationManager implements IManager {
        ...
        if (SystemTray.isSupported()) {
            try {
                if (trayIcon == null) {
                    ...
                    trayIcon = new TrayIcon(image, "cetrefilli.fun");
                    trayIcon.setImageAutoSize(true);
                    trayIcon.setToolTip("cetrefilli.fun");
                    tray.add(trayIcon);
                }
    ...
    private void mac(final String message) {
        ...
        processBuilder.command("osascript", "-e", "display notification \"" + message + "\" with title \"cetrefilli.fun\"");
        try {
    ...
    private void linux(final String message) {
        final ProcessBuilder processBuilder = new ProcessBuilder();
        processBuilder.command("notify-send", "-a", "cetrefilli.fun", message);
