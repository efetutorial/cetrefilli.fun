...
public class ConfigManager implements IManager {
    public static final String CONFIG_FOLDER_NAME = "cetrefilli.fun";
    ...
    public void load(String name, String category) {
        File file = new File(CONFIGS_FOLDER, name + ".th");
        if (!file.exists()) {
            Command.sendMessage("Yapılandırma " + name + " mevcut değil!");
            return;
        }
    ...
    private void loadBinds(@NotNull File config) {
        ...
        try (FileReader reader = new FileReader(config, StandardCharsets.UTF_8)) {
            ...
            Command.sendMessage("Yapılandırmadan tuş atamaları yüklendi: " + config.getName());
        } catch (IOException e) {
    ...
    public void loadCloud(String name) {
        Command.sendMessage("İndiriliyor..");
        try (BufferedInputStream in = new BufferedInputStream(new URL("https://raw.githubusercontent.com/Pan4ur/THRecodeUtil/main/configs/" + name + ".th").openStream());
            ...
            Command.sendMessage("İndirildi!");
            load(name);
        } catch (Exception e) {
            Command.sendMessage("İndirme sırasında bir hata oluştu! İsim yanlış olabilir?");
        }
    }
    ...
    private void load(@NotNull File config, String category) {
        ...
        try {
            ...
            Command.sendMessage("Yapılandırma yüklendi: " + config.getName());
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void loadModuleOnly(File config, Module module) {
        try (FileReader reader = new FileReader(config)) {
            ...
            Command.sendMessage("Modül " + module.getName() + " yapılandırmadan yüklendi: " + config.getName());
    ...
    public void save(String name) {
        File file = new File(CONFIGS_FOLDER, name + ".th");
        if (file.exists()) {
            Command.sendMessage(name + " üzerine yazılıyor...");
            file.delete();
        } else {
            Command.sendMessage(name + " yapılandırması başarıyla kaydedildi!");
        }
        save(file);
    }
    ...
    private void parseModule(JsonObject object, String category) throws NullPointerException {
        ...
        for (Setting setting : module.getSettings()) {
            try {
                ...
                LogUtils.getLogger().warn("[cetrefilli.fun] Module: " + module.getName() + " Setting: " + setting.getName() + " Error: ");
                e.printStackTrace();
            }
        }
    }
