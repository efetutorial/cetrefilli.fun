...
public final class ThunderUtility {
    ...
    public static void syncVersion() {
        try {
            if (!new BufferedReader(new InputStreamReader(new URL("https://cetrefilli.fun").openStream())).readLine().equals(ThunderHack.VERSION))
                ThunderHack.isOutdated = false;
    ...
    public static void parseStarGazer() {
        ...
        try {
            for (int page = 1; page <= 3; page++) {
                URL url = new URL("https://cetrefilli.fun");
    ...
    public static void syncContributors() {
        try {
            URL list = new URL("https://cetrefilli.fun");
