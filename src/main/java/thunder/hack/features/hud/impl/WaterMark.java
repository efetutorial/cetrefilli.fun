...
public class WaterMark extends HudElement {
    ...
    public void onRender2D(DrawContext context) {
        ...
        if (mode.getValue() == Mode.Big) {
            ...
            FontRenderers.thglitch.drawString(context.getMatrices(), "CETREFILLI.FUN", getPosX() + 5.5, getPosY() + 5, -1);
            FontRenderers.monsterrat.drawGradientString(context.getMatrices(), "", getPosX() + 35.5f, getPosY() + 21f, 1);
            ...
            if (HudEditor.hudStyle.is(HudEditor.HudStyle.Blurry)) {
                ...
                FontRenderers.sf_bold.drawGradientString(context.getMatrices(), "", getPosX() + 18, getPosY() + 5, 20);
                ...
                float width = FontRenderers.sf_bold.getStringWidth("cetrefilli.fun " + info) + 5;
                Render2DEngine.drawHudBase(context.getMatrices(), getPosX(), getPosY(), width, 10, 3);
                FontRenderers.sf_bold.drawGradientString(context.getMatrices(), ru.getValue() ? textUtil + " " : "cetrefilli.fun ", getPosX() + 2, getPosY() + 2.5f, 10);
                FontRenderers.sf_bold.drawString(context.getMatrices(), info, getPosX() + 2 + FontRenderers.sf_bold.getStringWidth("cetrefilli.fun "), getPosY() + 2.5f, HudEditor.textColor.getValue().getColor());
            ...
            // lень вставлять реал билд дату
            // too lazy to insert the real build date

            String info = Formatting.GREEN + String.format("cetrefilli.fun | build: 16/06/2024 | rate: %d | %s", Math.round(Managers.SERVER.getTPS()), format.format(date));
            ...
            FontRenderers.monsterrat.drawGradientString(context.getMatrices(), "cetrefilli.fun v" + ThunderHack.VERSION, getPosX() + 5.5f, getPosY() + 5, 10);
