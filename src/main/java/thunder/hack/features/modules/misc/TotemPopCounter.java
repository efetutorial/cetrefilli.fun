...
public class TotemPopCounter extends Module {
    ...
    public void onTotemPop(@NotNull TotemPopEvent event) {
        ...
        else s = Formatting.GREEN + event.getEntity().getName().getString() + Formatting.WHITE + " " + Formatting.AQUA + (event.getPops() > 1 ? event.getPops() + "" + Formatting.WHITE + " totem kullandı!" : Formatting.WHITE + "bir totem kullandı!");
    ...
    public void onUpdate() {
        for (PlayerEntity player : mc.world.getPlayers()) {
            ...
            else s = Formatting.GREEN + player.getName().getString() + Formatting.WHITE + " " + (Managers.COMBAT.popList.get(player.getName().getString()) > 1 ? Managers.COMBAT.popList.get(player.getName().getString()) + "" + Formatting.WHITE + " totem kullandı ve öldü!" : Formatting.WHITE + "bir totem kullandı ve öldü!");
