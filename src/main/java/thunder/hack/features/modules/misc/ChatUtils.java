...
public class ChatUtils extends Module {
    ...
    private final String[] qq = new String[]{
            ...
            "Hey, ",
            "İyi akşamlar, ",
            "SERVERIP1D5A9E sunucusuna hoş geldin, "
    };

    private final String[] popMessages = new String[]{
            " EZZZ <pop> ADET TOTEM KULLANDI REZIL",
            " ez <pop> adet totem kullandı beceriksiz",
            " <pop> adet totem kullandı git biraz öğren ",
            " EZZZZZZZ <pop> adet totem kullandı GİT PVP ÖĞRENİN ZAYIF",
            " rezalet <pop> adet totem kullanmış çok kolay",
            " hayatımda gördüğüm en kolay <pop> totem",
            " HAHAHAHA DOSTUM <pop> ADET TOTEM KULLANDI ÇOK KOLAY",
            " <pop> ADET TOTEM KULLANDI TANRIM ÇOK KÖTÜSÜN",
            " büyükannem senden daha yetenekli <pop> totem kullandı",
            " çöp <pop> adet totem kullandı aptal ",
            " ezz yetenek yok köpek <pop> adet totem",
            " zavallı benimle pvp yapmaya çalışıyor ama ölüyor) hahah <pop> totem",
            " daha iyi ol cidden <pop> totem",
            " <pop> adet totem kullandı gözlerin doğru çalışmıyor mu? ",
            " utanç verici <pop> totem kullandı çok kolay "
    };
        ...
        if (event.getPacket() instanceof GameMessageS2CPacket pac) {
            ...
            if (mention.getValue()) {
                if (pac.content.getString().contains(mc.player.getName().getString()) && messageTimer.passedMs(1000)) {
                    Managers.NOTIFICATION.publicity("ChatUtils", isRu() ? "Тебя помянули в чате!" : "Sohbette adınız geçti!", 4, Notification.Type.WARNING);
    ...
    public void onPacketSend(PacketEvent.@NotNull Send e) {
        if (e.getPacket() instanceof ChatMessageC2SPacket pac) {
            if (antiCoordLeak.getValue() && pac.chatMessage.replaceAll("\\D", "").length() >= 6) {
                sendMessage("[ChatUtils] " + (isRu() ? "В сообщении содержатся координаты!" : "Mesaj koordinat içeriyor!"));
                e.cancel();
            }
