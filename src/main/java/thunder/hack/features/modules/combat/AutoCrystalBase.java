...
public class AutoCrystalBase extends Module {
    ...
    public void onTick(EventTick e) {
        ...
        if (!InventoryUtility.findBlockInHotBar(Blocks.OBSIDIAN).found() && disableNoObby.getValue()) {
            disable(isRu() ? "Нет обсидиана!" : "Obsidyen yok!");
            return;
        }
        ...
        if (placeTimer.every(placeDelay.getValue()) && bestData != null && obbyResult.found() && isWorth()) {
            ...
            if (notification.getValue()) {
                ...
                if (isRu())
                    ...
                    content = "Obsidyen yerleştiriliyor:" + Formatting.GRAY + " X:" + bestData.position().getX() + " Y:" + bestData.position().getY() + " Z:" + bestData.position().getZ() + Formatting.WHITE + " hasar şu kadar artacak: " + Formatting.RED + MathUtility.round2(bestData.damage - ModuleManager.autoCrystal.renderDamage);
